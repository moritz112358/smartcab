...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None):
        global cycle_count, prev_action, prev_reward, cumul_rew
    ...
    def update(self, t):
        global alpha, gamma, cycle_count, Q_hat, prev_action, prev_reward, epsilon, cumul_rew
        ...
        # accumulate rewards, count negative rewards
        cumul_rew += reward
        if reward < 0:
            print "Negative_reward occurred"
        ...
        Q_hat[(prev_state, prev_action)] = (1 - curr_alpha) * Q_hat[(prev_state, prev_action)] + curr_alpha * (prev_reward + gamma * Q_hat[(self.state,action)])
        
        # update cycle_count for decaying alpha, epsilon
        cycle_count += 1
        
        # store previous action, reward
        prev_action = action
        prev_reward = reward
                ...
                for epsilon in [0.05]:
                    for decay_epsilon in [0.5]:
                        for multiple_runs in range(100):
                            ...
                            #global Q_hat
                            #pprint.pprint(Q_hat)
                            print "Alpha:", alpha, "Alphadecay:", decay_alpha, " Gamma:", gamma, " Epsilon:",epsilon, "Epsilondecy:", decay_epsilon, "CumulRew:", cumul_rew
